...
def parse_args():
    parser = ArgumentParser(description='DDNet for Deformation Detection')
    # model and dataset
    parser.add_argument('--model', type=str, default="DDNet", help="model name: (default DDNet)")
    parser.add_argument('--dataRootDir', type=str, default=r"dataset",
                        help="dataset dir")
    parser.add_argument('--dataset', type=str, default="deformationDetection", help="dataset for normalized deformation pairs")
...
def train_model(args):
    ...
    # Use MSE loss for deformation detection
...
def val(args, val_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        ...
        gt = np.asarray(label.numpy(), dtype=np.float32)  # Ensure ground truth is float32 for deformation
